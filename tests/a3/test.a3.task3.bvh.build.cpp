...
static void expect_bvh(std::vector<Vec3> &verts, size_t max_leaf_size, float exp_overlap = 1.99f)
{
	...
	{
		info("root_l_sa + root_r_sa = %f + %f > exp_overlap * root_sa = %f * %f (bvh.root_idx %d, max_size %d, l_id: %d, r_id: %d)",
			 root_l_sa, root_r_sa, exp_overlap, root_sa, bvh.root_idx, max_leaf_size, bvh.nodes.at(bvh.root_idx).l, bvh.nodes.at(bvh.root_idx).r);
