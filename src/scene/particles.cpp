...
bool Particles::Particle::update(const PT::Aggregate &scene, Vec3 const &gravity, const float radius, const float dt)
{
	...
	while (remain_t > EPS_F)
	{
		...
		auto ret = scene.hit(p_ray);
		float collision_t;

		if (ret.hit)
		{
			Vec3 normal = ret.normal.unit();
			if (dot(normal, velocity) < 0)
			{
				normal = -normal;
			}
			collision_t = ret.distance / velocity.norm() - radius / (velocity.norm() * dot(normal, velocity.unit()));
			// std::cout << dt << ", " << collision_t << "; " << ret.distance / velocity.norm() << "- " << radius / (velocity.norm() * dot(ret.normal.unit(), velocity.unit())) << "\n";
			if (collision_t < remain_t && collision_t > 0)
			{
		...
		velocity += gravity * collision_t;
		// (4) Repeat until the entire time step has been consumed.
		remain_t -= collision_t;
	}
	position += velocity * remain_t;
	velocity += gravity * remain_t;
	// std::cout << "p " << position << ";  v " << velocity << "\n";
...
{
	return a.gravity != b.gravity || a.radius != b.radius || a.initial_velocity != b.initial_velocity || a.spread_angle != b.spread_angle || a.lifetime != b.lifetime || a.rate != b.rate || a.step_size != b.step_size || a.seed != b.seed;
}
