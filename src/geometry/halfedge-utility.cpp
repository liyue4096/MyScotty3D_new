...
std::string Halfedge_Mesh::my_describe_R() const {
	...
	oss << "Mesh with " << halfedges.size() << " halfedges, " << vertices.size() << " vertices, " << edges.size() << " edges, and " << faces.size() << " faces:\n";
	for (auto const &h : halfedges) {
		oss << "  h" << h.id << "= list( t=\"" << desc_halfedge(h.twin)
		    << "\", n=\"" << desc_halfedge(h.next)
		    << "\", v= \"" << desc_vertex(h.vertex)
			<< "\", e_f = \"" << desc_edge(h.edge)
			<< " " << desc_face(h.face)
		    << "\"), " << "\n";
	}
	for (auto const &v : vertices) {
		oss << "  v" << v.id << "= c( " << v.position.x << ", " << v.position.y << ", "<< v.position.z << "),\n";
	}
	for (auto const &e : edges) {
		oss << "  [e" << e.id << "] " << desc_halfedge(e.halfedge) << "\n";
	}
