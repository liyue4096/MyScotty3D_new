...
struct BBox
{
	...
	bool hit(const Ray &ray, Vec2 &times) const
	{
		...
		// info("{%f, %f, %f}, {%f, %f, %f}", min.x, min.y, min.z, max.x, max.y, max.z);
		Vec3 invdir = 1.f / ray.dir;

		// info("origin %f, %f, %f ; dir: %f, %f, %f", ray.point.x, ray.point.y, ray.point.z, invdir.x, invdir.y, invdir.z);
		...
		// info("tmin %f, tmax %f, tymin %f, tymax %f, tzmin %f, tzmax %f", tmin, tmax, tymin, tymax, tzmin, tzmax);
		// info("ray.dist_bounds.x %f, ray.dist_bounds.y %f", ray.dist_bounds.x, ray.dist_bounds.y);

		if (tmin < times.x || tmax > times.y || tmax < times.x || tmin > times.y)
		{
			return false;
		}
